...
namespace cameval {
    ...
    void OpenMvgParser::parse() {
        ...
        std::map<int, cameval::CameraType> extrinsics;
    ...
    void OpenMvgParser::parseViews(const std::map<int, glm::mat3> & intrinsics, const std::map<int, cameval::CameraType> & extrinsics) {
        ...
        try {
            ...
            sfm_data_.camerasList_.assign(sfm_data_.numCameras_, cameval::CameraType());
            ...
            for (rapidjson::SizeType curCam = 0; curCam < camerasJson.Size(); curCam++) {
                ...
                try {
                    cameval::CameraType camCurrent = extrinsics.at(idExtrinsics);
    ...
    void OpenMvgParser::parseExtrinsics(std::map<int, cameval::CameraType> & extrinsics) {
      ...
      try {
        ...
        for (rapidjson::SizeType curInt = 0; curInt < extrinsicsJson.Size(); curInt++) {
          ...
          extrinsics.insert(std::pair<int, cameval::CameraType>(key, temp));
        }
